...
>
> ナビアプリの案内に例えると：
> - 命令型: 「100m先を右折。次の信号を左折。200m直進して右折…」（手順を逐一指示）
> - 関数型: 「東京駅に行きたい」（目的を宣言。経路の計算はナビに任せる）
>
> 関数型スタイルでは「何をしたいか」を書き、「どうやるか」の詳細はコンピュータ（またはライブラリ）に委ねます。
...
>
> 自動販売機を想像してください。
> - 100円を入れてボタンAを押せば → 必ずジュースAが出てくる
> - 前の人が何を買ったかや、機械の「気分」は関係ない
>
> これが純粋関数です。「同じ入力 → 常に同じ出力」で、内部で何か別のものを変えることもありません。
> 逆に、在庫切れで別の商品が出てきたり、購入のたびに売上データベースを更新したりする処理は「不純」な要素を持ちます。
...
>
> 数学の計算式を考えてください。
> `2 + 3` は `5` で置き換えても意味が変わりません。
> `(2 + 3) × 4` は `5 × 4` と書いても同じです。
>
> プログラムでも、純粋関数の呼び出しは「その結果の値」で置き換えられます。
> これが参照透過性です。
>
> 参照透過性がない例：「今日の日付を返す関数」は、呼ぶたびに値が変わるので、結果の値で置き換えることができません。
...
> **身近なアナロジー**
>
> 「合計金額を計算する」という処理があるとします。
> - 純粋な処理: 数字のリストを受け取って合計値を返す（計算だけ）
> - 副作用: レジのプリンターでレシートを印刷する、在庫データベースを更新する
>
> 副作用自体は悪いものではなく（レシートは必要です）、問題は**副作用が「こっそり」起きること**です。
> 関数型スタイルでは副作用をできるだけ関数の外側（プログラムの境界）に押し出し、内側のロジックを純粋に保ちます。
